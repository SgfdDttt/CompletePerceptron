...
(*Cet algorithme renvoie le résultat de n itérations du perceptron avec la marge obtenue, en gardant en mémoire la meilleure solution rencontrée jusque là. La recursivité terminale est nécessaire pour éviter le Out_of_memory. Si le vecteur solution passe par 0, c'est inclassifiable d'après Gordan.*)
...
let train_sample T =
let p = vect_length T in
let N = vect_length T.(0) in
let v = make_matrix 5 N 0. in
  for k = 0 to 4 do
    v.(k) <- T.(20*k) ;
  done ;
v ;;
...
let p = vect_length T in
let N = vect_length T.(0) in
let v = make_matrix (9*p/10) N 0. in
let x = ref 0 in
  for k = 0 to 99 do
    if k mod 10 <> 0 then
      begin
...
let taux_de_réussite A w =
let p = vect_length A in
let v = make_vect p 0 in
  for k = 0 to p-1 do
    if scal A.(k) w > 0. then v.(k) <- 1 else v.(k) <- -1
  done ;
let x = ref 0 in
  for k = 0 to p-1 do
    if v.(k) = 1 then x := !x + 1
...
let C = train_sample A ;;
let D = test_sample A ;;
vect_length C ;;
vect_length D ;;

let w,a,k = algorithme_pal_rujan C ;;
taux_de_réussite D w ;;
